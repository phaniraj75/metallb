...
var _ = ginkgo.Describe("L2", func() {
	...
	ginkgo.Context("type=Loadbalancer", func() {
		...
		ginkgo.It("should work for ExternalTrafficPolicy=Local", func() {
			...
			err := jig.Scale(context.Background(), 5)
			framework.ExpectNoError(err)

			epNodes, err := jig.ListNodesWithEndpoint(context.Background()) // Only nodes with an endpoint could be advertising the IP
			framework.ExpectNoError(err)
		...
		ginkgo.It("IPV4 Should work with mixed protocol services", func() {
			...
			svc1, err := jig1.CreateLoadBalancerService(context.Background(), loadBalancerCreateTimeout, func(svc *corev1.Service) {
			...
			framework.ExpectNoError(err)
			_, err = jig1.Run(context.Background(),
	...
	ginkgo.DescribeTable("different services sharing the same ip should advertise from the same node", func(ipRange *string) {
		...
		svc2, err := jig2.CreateLoadBalancerService(context.Background(), loadBalancerCreateTimeout, func(svc *corev1.Service) {
		...
		framework.ExpectNoError(err)
		_, err = jig2.Run(context.Background(),
			func(rc *corev1.ReplicationController) {
